...
let BookingKuma = require('../models/booking.kuma.model');
...
router.route('/add').post((req, res) => {
    ...
    const newBooking = new BookingKuma({
        name: req.body.name,
    ...
    newBooking.save()
        .then((r) => {
            console.log('booking-saved-kuma:', r);
            ...
            let mailDetails = {
                ...
                to: process.env.KUMA_EMAIL,
                subject: 'New Reservation Kuma Kitchen and Bar',
                html: "<div>" +
                    ...
                    "<p><a href='" + process.env.UI_URL + "/kuma/reservations/" +  sDayUnix + "'>See Calendar</a></p>" +
                    "</div>"
            };
...
router.route('/delete/:id').post((req, res) => {
    ...
    BookingKuma.findByIdAndUpdate(req.params.id, { deleted: req.body.delete || false })
...
router.route('/update/:id').post((req, res) => {
    ...
    BookingKuma.findByIdAndUpdate(req.params.id, req.body).then((r) => res.json(`${req.params.id} updated`))
        .catch((e) => {
